      ...
      ${i18n.products.map((product) => `
        <div class="quote-product-row" role="row" data-product-id="${escapeHtml(product.id)}">
          <label class="quote-check-cell" aria-label="${escapeHtml(product.name)}">
            <input type="checkbox" class="quote-product-checkbox" value="${escapeHtml(product.id)}">
          </label>
          ...
          <div class="quote-case-controls">
            <select class="quote-case-select" disabled>
              ${optionMarkup}
            </select>
...
function setupQuoteModal() {
  ...
  elements.openButton.addEventListener("click", openQuoteModal);
  ...
  elements.modal.querySelectorAll("[data-quote-close]").forEach((element) => {
    element.addEventListener("click", closeQuoteModal);
  });
